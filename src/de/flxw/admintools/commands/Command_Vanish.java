        ...
        if(commandSender instanceof Player)
        {
            ...
            if(player.hasPermission(PERM_VANISH) || player.hasPermission(AdminTools.getInstance().PERM_ALL))
            {
                ...
                else if(args.length == 1)
                {
                    ...
                    if(target != null)
                    {
                        if(!ArrayLists.bypassVanish.contains(target))
                        {
                            if(!ArrayLists.vanish.contains(target))
                            {
                                ArrayLists.vanish.add(target);
                                for (Player all : Bukkit.getOnlinePlayers())
                                {
                                    all.hidePlayer(target);
                                }
                                target.sendMessage(AdminTools.getInstance().Prefix + AdminTools.getInstance().VanishMessage);
                                player.sendMessage(AdminTools.getInstance().Prefix + AdminTools.getInstance().OtherVanish.replace("%player%", target.getName()));
                            }
                            else
                            {
                                ArrayLists.vanish.remove(target);
                                for (Player all : Bukkit.getOnlinePlayers())
                                {
                                    all.showPlayer(target);
                                }
                                target.sendMessage(AdminTools.getInstance().Prefix + AdminTools.getInstance().NoLongerVanishMessage);
                                player.sendMessage(AdminTools.getInstance().Prefix + AdminTools.getInstance().OtherNoLongerVanish.replace("%player%", target.getName()));
                            }
                        }
                        else
                        {
